#!/usr/bin/env python3
"""
sn2_500600kb_series.py — sn2_chaos8_runs.py, plus real melodic material
pulled from 7 found MIDI files (~560-615KB each, hence the name) instead of
purely algorithmic runs. Sibling of sn2_300kb_series.py and
sn2_400kb_series.py, built the same way from a different batch of source
files -- each "series" is its own independent line, not layered on the
others.

FOUND-FLOOR PHRASES: found_floor_phrases_500600kb.json (built once from
tasted toasts one midi.mid, fifth light world 1q.mid, crition axix 2.mid,
new tecite 1x.MID, how berry is very.mid, forming at one 5.mid, and now is
won backwards 3b.mid) holds ~4,800 short melodic phrases as
relative-semitone patterns, up to 700 drawn evenly from each source file
(crition axix 2.mid only had 614 unique phrases after dedup, so it
contributes fewer than the others). Both the SN2's wandering run and a new
...
Run:      python3 sn2_500600kb_series.py
"""
import rtmidi
import time
import math
...
PHRASE_POOL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "found_floor_phrases_500600kb.json")
...
def main():
    ...
    print(f"\n500-600kb series -- {NUM_CHANNELS} SN2 channels of random notes + mod wheel, "
