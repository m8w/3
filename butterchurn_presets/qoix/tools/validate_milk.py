...
def validate(path: Path) -> list[str]:
    ...
    # Functions the parsers reject. Each was confirmed against the real
    # converter, and each fails the WHOLE preset, not just one line:
    #   milkdrop-eel-parser has no mod() - modulo is the % operator
    #   hlslparser-js has no mod() or distance()
    for name in ("per_frame", "per_pixel"):
        for n, body in sections.get(name, {}).items():
            if re.search(r"\bmod\s*\(", body):
                errs.append(f"{name}_{n}: EEL has no mod() - use the % operator "
                            f"('a % b'); mod() fails the entire preset")
            if re.search(r"(?<![A-Za-z0-9_])_[A-Za-z]", body):
                errs.append(f"{name}_{n}: identifier starting with '_' - "
                            f"milkdrop-eel-parser rejects these and the whole "
                            f"per_frame block is lost")

    for shader in ("warp", "comp"):
        entries = sections.get(shader)
        if not entries:
            continue
        body = "\n".join(v[1:] if v.startswith("`") else v
                         for _, v in sorted(entries.items()))
        body = re.sub(r"//.*", "", body)
        for fn in ("mod", "distance"):
            if re.search(rf"\b{fn}\s*\(", body):
                alt = "fmod" if fn == "mod" else "length(a - b)"
                errs.append(f"{shader}: HLSL has no {fn}() here - use {alt}. "
                            f"The converter emits an EMPTY shader, silently.")
